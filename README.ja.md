...
本サイトは Progressive Web App（PWA）として動作するため、iOS / Android の
ホーム画面に追加してスタンドアロン（ブラウザ UI のない全画面）モードで
起動できます。Service Worker がアプリシェルをプリキャッシュするので、
再起動も高速です。

- **Android（Chrome）:** デモ URL を開き、ブラウザのメニューから
  「アプリをインストール」（または「ホーム画面に追加」）を選択します。
- **iOS（Safari）:** デモ URL を開き、共有ボタンから
  「ホーム画面に追加」を選択します。

ルーム自体は WebRTC による P2P 通信なので、起動はオフラインでも可能
ですが、ルーム参加・共有にはネットワーク接続が必要です。

**画面の向きについて:** マニフェストでは画面向きを固定・上書きしない
ため、端末側の自動回転 / 画面ロック設定にそのまま従います（例: Android
で自動回転を OFF にしていれば、PWA として起動したアプリも端末を傾けて
向きが変わることはありません）。
