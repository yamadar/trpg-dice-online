import type { Dict } from '../translations'

export const ja: Dict = {
  'app.title': 'Dice & Chat',
  'common.confirm': '実行',
  'common.cancel': 'キャンセル',
  'common.apply': '適用',
  'common.confirmDialog': '確認',
  'app.tagline': 'オンラインTRPGセッションを簡単に',
  'lang.label': '言語',
  'translate.auto': '自動翻訳',
  'theme.title': 'テーマ',
  'theme.midnight': 'ミッドナイト',
  'theme.forest': 'フォレスト',
  'theme.ember': 'エンバー',
  'theme.rose': 'ローズ',
  'theme.daylight': 'デイライト',
  'theme.parchment': 'パーチメント',
  'fontSize.label': '文字サイズ',
  'fontSize.small': '小',
  'fontSize.medium': '中',
  'fontSize.large': '大',

  'settings.title': '設定',
  'settings.open': '設定',
  'settings.close': '閉じる',
  'settings.groupLanguage': '言語と翻訳',
  'settings.groupAppearance': '外観',
  'settings.groupTyping': '入力中の表示',
  'settings.showTyping': '他の人の入力中を見る',
  'settings.broadcastTyping': '自分の入力中を他者に通知',
  'settings.about': 'このアプリについて',

  'dock.room': 'ルーム',
  'dock.character': 'キャラ',
  'dock.dice': 'ダイス',
  'dock.nav': 'ナビゲーション',
  'dock.tabletop': 'マップ',
  'tabletop.title': 'テーブルマップ',
  'tabletop.grid.title': 'グリッド',
  'tabletop.grid.kind': '種類',
  'tabletop.grid.kindNone': 'なし',
  'tabletop.grid.kindSquare': 'スクエア',
  'tabletop.grid.kindHex': 'ヘックス',
  'tabletop.grid.cellSize': 'セルサイズ',
  'tabletop.grid.originX': '原点 X',
  'tabletop.grid.originY': '原点 Y',
  'tabletop.grid.strokeColor': '線色',
  'tabletop.grid.strokeOpacity': '透明度',
  'tabletop.grid.snap': 'グリッドにスナップ',
  'tabletop.map.title': '背景マップ',
  'tabletop.map.choose': '画像を選ぶ…',
  'tabletop.map.replace': '画像を変更…',
  'tabletop.map.clear': 'マップを外す',
  'tabletop.map.set': 'マップを更新しました',
  'tabletop.map.tooLarge': '画像が大きすぎます（最大 8 MB）',
  'tabletop.map.unreadable': '画像を読み込めませんでした',
  'tabletop.gmToken.title': 'NPC・モンスター',
  'tabletop.gmToken.add': 'トークンを追加…',
  'tabletop.gmToken.labelPlaceholder': 'ラベル（任意）',
  'tabletop.gmToken.remove': '削除',
  'tabletop.gmToken.added': 'トークンを追加しました',
  'tabletop.gmToken.unreadable': '画像を読み込めませんでした',
  'tabletop.gmToken.unlabeled': '（ラベルなし）',
  'tabletop.panel.title': 'テーブル操作',
  'tabletop.panel.tokens': 'トークン',
  'tabletop.panel.nav': 'ツールバーのカテゴリ',
  'tabletop.panel.close': 'パネルを閉じる',
  'tabletop.playerToken.title': 'プレイヤートークン',
  'tabletop.playerToken.choose': '参加者を選ぶ…',
  'tabletop.playerToken.add': 'プレイヤートークンを追加',
  'tabletop.playerToken.added': 'トークンを追加しました',
  'tabletop.playerToken.allPlaced': '参加者全員に既にトークンがあります',
  'tabletop.tokenEdit.titleGm': 'NPC トークン',
  'tabletop.tokenEdit.titlePc': 'PC トークン',
  'tabletop.tokenEdit.label': 'ラベル',
  'tabletop.tokenEdit.note': 'メモ',
  'tabletop.tokenEdit.sharedNote': '共有メモ',
  'tabletop.tokenEdit.privateNote': 'GMメモ（非公開）',
  'tabletop.tokenEdit.changeImage': '画像を変更…',
  'tabletop.tokenEdit.size': 'サイズ',
  'tabletop.tokenEdit.facing': '向き',
  'tabletop.tokenEdit.facingNone': '向きなし',
  'tabletop.tokenEdit.facingDir': '{deg}° の向き',
  'tabletop.tokenEdit.hp': 'HP',
  'tabletop.tokenEdit.hpCurrent': '現在 HP',
  'tabletop.tokenEdit.hpMax': '最大 HP',
  'tabletop.tokenEdit.hpClear': 'HP をクリア',
  'tabletop.tokenEdit.statuses': '状態異常',
  'tabletop.status.poison': '毒',
  'tabletop.status.stun': 'スタン',
  'tabletop.status.sleep': '睡眠',
  'tabletop.status.fear': '恐怖',
  'tabletop.status.charm': '魅了',
  'tabletop.status.burn': '燃焼',
  'tabletop.status.freeze': '凍結',
  'tabletop.status.bless': '祝福',
  'tabletop.status.shield': '防御',
  'tabletop.status.haste': '加速',
  'tabletop.status.bleed': '出血',
  'tabletop.status.down': 'ダウン',
  'tabletop.tokenEdit.remove': 'トークンを消す',
  'tabletop.tokenEdit.editCharacter': 'キャラクター情報',
  'tabletop.tokenEdit.canOperate': '{names}が操作できます',
  'tabletop.reorder.up': '上へ',
  'tabletop.reorder.down': '下へ',
  'tabletop.tokenEdit.close': '閉じる',
  'tabletop.playerToken.place': '配置',
  'tabletop.playerToken.placed': '配置済み',
  'tabletop.playerToken.alreadyPlaced': 'このキャラクターは既にマップ上に配置されています',
  'tabletop.playerToken.noCharacters': 'まずキャラクターを作成してください',
  'tabletop.npcLibrary.title': 'NPC ライブラリ',
  'tabletop.npcLibrary.add': 'NPC を追加',
  'tabletop.npcLibrary.addShort': '追加',
  'tabletop.npcLibrary.place': '配置',
  'tabletop.npcLibrary.remove': 'ライブラリから削除',
  'tabletop.npcLibrary.unreadable': '画像を読み込めませんでした',
  'tabletop.npcLibrary.setImage': '画像を設定…',
  'tabletop.npcLibrary.changeImage': '画像を変更…',
  'tabletop.npcLibrary.imageUpdated': 'NPC の画像を更新しました',
  'tabletop.npcLibrary.edit': 'NPC を編集',
  'tabletop.npcLibrary.editTitle': 'NPC を編集',
  'tabletop.npcLibrary.newTitle': 'NPC を作成',
  'tabletop.npcLibrary.nameLabel': '名前',
  'tabletop.npcLibrary.confirmDelete': 'NPC ライブラリから「{name}」を削除しますか？',
  'tabletop.placedTokens.title': '配置済みトークン',
  'tabletop.onMap.title': 'マップ上のトークン',
  'tabletop.setup.title': '追加・準備',
  'tabletop.placedTokens.empty': 'まだトークンは配置されていません',
  'tabletop.placedTokens.kindPc': 'PC',
  'tabletop.placedTokens.kindGm': 'NPC',
  'tabletop.placedTokens.remove': 'マップから削除',
  'tabletop.placedTokens.unnamed': '（名前なし）',
  'tabletop.library.title': 'テーブルライブラリ',
  'tabletop.library.saveCurrent': '現在のテーブルを保存',
  'tabletop.library.namePlaceholder': 'テーブル名',
  'tabletop.library.saveAsTemplate': 'テンプレートとして保存',
  'tabletop.library.saveAsSave': 'スナップショットとして保存',
  'tabletop.library.saveHint': 'テンプレートは初期配置を保存します（PC トークンとペン描画は除外、テキストとフォグは保持）。スナップショットはすべてをそのまま保存します。「テーブルを置換」はテーブル全体を入れ替え、「シーンとして追加」は現在のシーンに加えます。',
  'tabletop.library.scopeNote': 'この端末に保存され、どの部屋でも再利用できます。シーン（このゲーム内だけ）とは別物です。',
  'tabletop.library.help': '保存と読み込みの説明',
  'tabletop.library.scope': '保存範囲',
  'tabletop.library.scopeScene': 'このシーン',
  'tabletop.library.scopeTable': 'テーブル全体（全シーン）',
  'tabletop.library.loadReplace': 'テーブルを置換',
  'tabletop.library.loadAsScenes': 'シーンとして追加',
  'tabletop.library.overwrite': '上書き保存',
  'tabletop.library.confirmOverwrite': '「{name}」を現在のテーブルで上書きしますか？',
  'tabletop.library.overwritten': '上書きしました',
  'tabletop.library.sceneCountBadge': '{n} シーン',
  'tabletop.library.addedAsScenes': 'シーンとして追加しました。',
  'tabletop.library.templates': 'テンプレート',
  'tabletop.library.saves': 'スナップショット',
  'tabletop.library.emptyTemplates': 'テンプレートはまだありません',
  'tabletop.library.emptySaves': 'スナップショットはまだありません',
  'tabletop.library.load': 'ロード',
  'tabletop.library.delete': '削除',
  'tabletop.library.confirmDelete': '「{name}」を削除しますか？',
  'tabletop.library.needName': '先に名前を入力してください',
  'tabletop.library.savedTemplate': 'テンプレートを保存しました',
  'tabletop.library.savedSave': 'スナップショットを保存しました',
  'tabletop.library.saveFailed': '保存に失敗しました',
  'tabletop.library.loaded': 'テーブルを読み込みました',
  'tabletop.library.loadFailed': '読み込みに失敗しました',
  'tabletop.library.deleted': '削除しました',
  'tabletop.dock.nav': 'テーブルマップ操作',
  'tabletop.dock.chat': 'チャット',
  'tabletop.dock.character': 'キャラ',
  'tabletop.dock.dice': 'ダイス',
  'tabletop.dock.returnToRoom': 'ルームに戻る',
  'tabletop.help': 'チュートリアル',
  'tabletop.tutorial.welcome.title': 'テーブルマップへようこそ',
  'tabletop.tutorial.welcome.body':
    'セッションの共有キャンバスです。トークンを動かし、描き込み、ピンを落とせます。GM はフォグや複数シーンも管理できます。デスクトップでもスマホでも快適に操作できます。',
  'tabletop.tutorial.viewport.title': 'マップを動かす・拡大する',
  'tabletop.tutorial.viewport.body':
    '何もない場所をドラッグで視点移動。スクロールか 2 本指ピンチで拡大／縮小（25〜400%）。左下のミニマップにシーン全体が表示され、タップやドラッグでその場所へジャンプできます。自分の視点だけが動き、他の参加者には影響しません。キーボードの「?」でショートカット一覧を開けます。',
  'tabletop.tutorial.tools.title': '左の道具パレット',
  'tabletop.tutorial.tools.body':
    '選択／ドラッグ、テキスト、ペン、消しゴム、ピン（1〜5 キーで切り替え）。ピンは誰でも使える「ここを見て」マーカー、テキストとペンは色や太さを変更できます。GM 限定で、フォグの「表示／隠す」ブラシでセル単位に視界を開閉できます。',
  'tabletop.tutorial.tokens.title': 'トークン — 移動・タップ・操作',
  'tabletop.tutorial.tokens.body':
    '自分のトークンはドラッグで移動できます。選択して矢印キーで動かすこともできます（Del で削除、[ ] でトークンを切り替え）。どのトークンもタップするとカードが開き、PC トークンにはミニキャラカード（タップで詳細）と全員が編集できる共有メモが表示されます。所有者または GM は、サイズ・向き・HP・状態異常をそれぞれの行で設定できます。',
  'tabletop.tutorial.rightPanel.title': '右側のパネル',
  'tabletop.tutorial.rightPanel.body':
    '右側のアイコン列から準備パネルを開きます。全員が使える「トークン」では、キャラの配置、マップ上のトークンのスポットライト、NPC の管理ができます。GM 限定で「背景マップ」「グリッド」「フォグ」「シーン」「テーブルライブラリ」があります。シーンは独立したマップで、それぞれにマップ・トークン・フォグを持ち、切り替えると全員の表示が変わります。',
  'tabletop.tutorial.dock.title': '下部メニュー',
  'tabletop.tutorial.dock.body':
    'チャット、キャラクターシート、ダイスロール。ダイスを振るとチャットアイコンに未読ドットが点くのでロールを見逃しません。スマホではチャットとダイスは交互表示で、ダイスを振ると自動でチャットに切り替わります。',
  'tabletop.tutorial.help.title': 'いつでも見直せます',
  'tabletop.tutorial.help.body':
    'アイコン列の一番下にある「？」ボタンから、このチュートリアルをいつでも再表示できます。デスクトップではキーボードの「?」でショートカット一覧を開けます。',
  'tabletop.preset.title': 'プリセットマップ',
  'tabletop.preset.choose': 'プリセットを選ぶ…',
  'tabletop.preset.load': 'プリセットを読み込む',
  'tabletop.preset.loaded': 'プリセットを読み込みました',
  'tabletop.preset.unreadable': 'プリセットの読み込みに失敗しました',
  'tabletop.preset.empty': '利用可能なプリセットはありません',
  'tabletop.mapUrl.title': '画像 URL から読み込む',
  'tabletop.mapUrl.placeholder': 'https://example.com/map.png',
  'tabletop.mapUrl.load': 'URL から読み込む',
  'tabletop.mapUrl.invalidUrl': 'URL が正しくありません',
  'tabletop.mapUrl.fetchFailed': '画像を取得できませんでした（CORS 制限の可能性）',
  'tabletop.mapUrl.notImage': '画像ではありません',
  'tabletop.gallery.title': 'マップギャラリー',
  'tabletop.gallery.open': 'ギャラリーを開く…',
  'tabletop.gallery.close': '閉じる',
  'tabletop.gallery.searchPlaceholder': 'ファイル名・説明で検索',
  'tabletop.gallery.mode': '検索モード',
  'tabletop.gallery.modeOr': 'いずれか',
  'tabletop.gallery.modeAnd': 'すべて',
  'tabletop.gallery.cat.theme': 'テーマ',
  'tabletop.gallery.cat.terrain': '地形',
  'tabletop.gallery.cat.mood': '雰囲気',
  'tabletop.gallery.cat.location': '場所',
  'tabletop.gallery.clearFilters': 'フィルタをクリア',
  'tabletop.gallery.clear': 'クリア',
  'tabletop.gallery.closeFilter': 'フィルタを閉じる',
  'tabletop.gallery.loading': 'ギャラリーを読み込み中…',
  'tabletop.gallery.loadFailed': 'ギャラリーを読み込めませんでした',
  'tabletop.gallery.count': '{shown} / {total} 件',
  'tabletop.gallery.empty': '該当するマップがありません',
  'tabletop.gallery.cancel': 'キャンセル',
  'tabletop.gallery.use': 'このマップを使う',
  'tabletop.gallery.applying': '読み込み中…',
  'tabletop.gallery.applyFailed': 'マップの適用に失敗しました',
  'tabletop.gallery.preview': '拡大プレビュー',
  'tabletop.mapSource.label': '読み込み元',
  'tabletop.mapSource.upload': 'アップロード',
  'tabletop.mapSource.gallery': 'ギャラリー',
  'tabletop.mapSource.url': 'URL',
  'tabletop.mapSource.preset': 'プリセット',
  'tabletop.mapSource.uploadDesc': '端末のファイルから読み込む',
  'tabletop.mapSource.galleryDesc': 'trpg-map-organizer のマップ集から選ぶ',
  'tabletop.mapSource.urlDesc': '画像の公開 URL を指定して取得',
  'tabletop.mapSource.presetDesc': 'テスト用のプリセット',
  'tabletop.imagePicker.title': '画像を選ぶ',
  'tabletop.imagePicker.close': '閉じる',
  'tabletop.imagePicker.source': '画像のソース',
  'tabletop.imagePicker.upload': 'アップロード',
  'tabletop.imagePicker.character': 'キャラ',
  'tabletop.imagePicker.monster': 'モンスター',
  'tabletop.imagePicker.uploadDesc': '端末のファイルから読み込む',
  'tabletop.imagePicker.characterDesc': 'trpg-chara-image-organizer のキャラクター画像から選ぶ',
  'tabletop.imagePicker.monsterDesc': 'trpg-chara-image-organizer のモンスター画像から選ぶ',
  'tabletop.imagePicker.choose': '画像を選ぶ…',
  'tabletop.imagePicker.applying': '適用中…',
  'tabletop.imagePicker.use': 'この画像を使う',
  'tabletop.imagePicker.loading': '読み込み中…',
  'tabletop.imagePicker.loadFailed': 'ライブラリを読み込めませんでした',
  'tabletop.imagePicker.fetchFailed': '画像を取得できませんでした',
  'tabletop.imagePicker.searchPlaceholder': '種族・職業・タグで検索',
  'tabletop.imagePicker.cat.race': '種族',
  'tabletop.imagePicker.cat.gender': '性別',
  'tabletop.imagePicker.cat.age': '年齢',
  'tabletop.imagePicker.cat.profession': '職業',
  'tabletop.tools.title': 'マップツール',
  'tabletop.tools.select': '選択／ドラッグ',
  'tabletop.tools.text': 'テキスト',
  'tabletop.tools.pen': 'ペン',
  'tabletop.tools.eraser': '消しゴム',
  'tabletop.tools.ping': 'ピン',
  'tabletop.tools.pingHint': 'マップをタップして「ここ！」ピンを送る',
  'tabletop.tools.pingOffscreen': '{name} のピン',
  'tabletop.shortcuts.title': 'キーボードショートカット',
  'tabletop.shortcuts.move': '選択トークンを移動',
  'tabletop.shortcuts.cycle': '前 / 次のトークン',
  'tabletop.shortcuts.tools': 'ツール切替',
  'tabletop.shortcuts.zoom': 'ズーム 拡大 / 縮小 / リセット',
  'tabletop.shortcuts.center': '選択へカメラを寄せる',
  'tabletop.shortcuts.remove': '選択トークンを削除',
  'tabletop.shortcuts.escape': '選択解除 / 閉じる',
  'tabletop.shortcuts.help': 'このヘルプを表示',
  'tabletop.scenes.title': 'シーン',
  'tabletop.scenes.hint': '各シーンは独自のマップ・トークン・フォグを持ちます。切り替えると全員の表示が変わります。',
  'tabletop.scenes.add': 'シーンを追加',
  'tabletop.scenes.switch': 'このシーンに切り替え',
  'tabletop.scenes.rename': '名前を変更',
  'tabletop.scenes.delete': 'シーンを削除',
  'tabletop.scenes.deleteConfirm': 'シーン「{name}」を削除しますか？',
  'tabletop.scenes.defaultName': 'シーン{n}',
  'tabletop.minimap.label': 'ミニマップ',
  'tabletop.minimap.collapse': 'ミニマップを隠す',
  'tabletop.minimap.show': 'ミニマップを表示',
  'tabletop.tools.fogReveal': '霧を晴らす',
  'tabletop.tools.fogConceal': '霧をかける',
  'tabletop.tools.fogDisabled': '先にツールバーからフォグを有効にしてください',
  'tabletop.tools.widthAdjust': 'ペンの太さを調整',
  'tabletop.tools.sizeAdjust': '文字サイズを調整',
  'tabletop.tools.closeSetting': '閉じる',
  'tabletop.tools.color': '色',
  'tabletop.tools.width': '太さ',
  'tabletop.tools.fontSize': 'サイズ',
  'tabletop.tools.textPlaceholder': '文字を入力し Enter で配置',
  'tabletop.tools.textOk': '配置',
  'tabletop.tools.textCancel': 'キャンセル',
  'tabletop.fog.title': 'フォグオブウォー',
  'tabletop.fog.enable': '霧を有効化',
  'tabletop.fog.disable': '霧を解除',
  'tabletop.fog.fillAll': '全てを霧で覆う',
  'tabletop.fog.clearAll': '全てを開示',
  'tabletop.fog.confirmFillAll': 'マップ全体を霧で覆いますか？',
  'tabletop.fog.confirmClearAll': 'マップ全体の霧を晴らしますか？',
  'tabletop.fog.needGrid': 'グリッド種類を選んでください',
  'tabletop.fog.brushHint':
    '左側のツールバーから「霧を晴らす」「霧をかける」を選ぶと、グリッド 1 マス単位で霧を追加・削除できます。',
  'tabletop.text.removeOwn': 'テキストを削除',
  'tabletop.stroke.removeOwn': 'ストロークを消す',
  'tabletop.error.title': 'テーブルマップでエラーが発生しました',
  'tabletop.error.body': 'マップの描画中に問題が起きました。ルームは接続されたままです。下のいずれかの操作で復帰してください。',
  'tabletop.error.retry': 'もう一度試す',
  'tabletop.error.clearMap': '背景マップを外して再開',
  'tabletop.error.close': 'テーブルを閉じる',
  'tabletop.error.details': 'エラーの詳細',
  'app.error.title': '予期しないエラーが発生しました',
  'app.error.body': 'アプリで予期しないエラーが発生しました。リロードすると復帰できることがほとんどです。保存済みデータは保持されます。',
  'app.error.reload': 'リロード',
  'app.error.details': 'エラーの詳細',


  'status.offline': 'オフライン',
  'status.noCharacter': 'なし（PL 本人）',

  'gate.start': 'はじめる',
  'gate.nameHint': 'プレイヤー名は設定からいつでも変更できます。',

  'toast.roomName': 'ルーム名を変更しました',
  'toast.roomCodeChanged': 'ルームコードを変更しました',
  'toast.characterName': 'キャラクター名を変更しました',
  'toast.characterDetail': 'キャラクター詳細を変更しました',
  'toast.playerName': 'プレイヤー名を変更しました',
  'toast.patternSaved': 'パターンを保存しました',
  'toast.patternUpdated': 'パターンを更新しました',

  'settings.help': '使い方を見る',
  'tutorial.skip': 'スキップ',
  'tutorial.next': '次へ',
  'tutorial.back': '戻る',
  'tutorial.done': 'はじめる',
  'tutorial.welcome.title': 'Dice & Chat へようこそ',
  'tutorial.welcome.body':
    '他のプレイヤーとダイスを振り、出目やチャットをリアルタイムに共有できます。インストール不要、ルームに入らずローカルでも使えます。',
  'tutorial.dice.title': 'ダイスを振る',
  'tutorial.dice.body':
    '下部ドックの「ダイス」から、個数・ダイス・補正・用途を選んで振ります。出目はフィードに記録され、GM は「隠しロール」で出目を伏せて振れます。',
  'tutorial.character.title': 'キャラクター',
  'tutorial.character.body':
    '「キャラ」でキャラクターを作成・切り替えできます。背景情報やメモを設定でき、キャラクターはファイルに書き出し／読み込みできます。',
  'tutorial.patterns.title': 'パターン',
  'tutorial.patterns.body':
    'よく使うダイスは「パターン」に名前を付けて保存し、一覧からワンタップで振れます。GM はパターンに隠しロールを設定できます。',
  'tutorial.room.title': 'ルームで共有',
  'tutorial.room.body':
    '「ルーム」でルームを作成（GM になる）するか、コードで参加します。GM はルームコードを指定・変更でき、通信が切れても自動で再接続します。GM はルームのホストになるため、GM がオフラインになるとほかのプレイヤーも切断されます。',
  'tutorial.pastRooms.title': '過去のルーム',
  'tutorial.pastRooms.body':
    '終了したルームは読み取り専用で残ります。「ルーム」画面の「過去のルーム」から開けます。1 件ずつ、または全件まとめて削除できます。',
  'tutorial.chat.title': 'チャットと添付',
  'tutorial.chat.body':
    'フィードでチャットができ、画像やファイルも添付できます。「@名前」で相手にメッセージを強調表示でき、フィードの名前をタップすると相手の情報を確認できます。',
  'tutorial.translate.title': 'チャットの自動翻訳',
  'tutorial.translate.body':
    '設定で「自動翻訳」を ON にすると、他プレイヤーのチャットが表示言語に翻訳されます。翻訳されたメッセージで「原文」をタップすると、相手が送ったままの文を確認できます。',
  'tutorial.settings.title': '設定とヘルプ',
  'tutorial.settings.body':
    'プレイヤー名・言語・カラーテーマの変更や、このガイドの再表示は、右上の設定（⚙）から行えます。',
  'tutorial.tabletop.title': '共有マップ',
  'tutorial.tabletop.body':
    '下部ドックの「マップ」をタップすると共有テーブルマップが開きます。グリッド上でトークンを動かし、描き込み、ピンを落とせます。GM はフォグや複数シーンも管理できます。開くと専用のミニツアーが表示されます。',

  'player.section': 'プレイヤー',
  'player.name': 'プレイヤー名',
  'player.namePlaceholder': 'あなたの名前',
  'player.anon': '名無し',

  'character.section': 'キャラクター',
  'character.activeLabel': '操作するキャラクター',
  'character.asPlayer': 'キャラクターなし（PL 本人）',
  'character.addSection': 'キャラクターを追加',
  'character.create': '新規作成',
  'character.empty': 'キャラクターがまだありません。作成してみましょう。',
  'character.name': 'キャラクター名',
  'character.namePlaceholder': '例: 戦士ガロン',
  'character.background': '背景情報（ルーム内に公開）',
  'character.backgroundPlaceholder': '種族・職業・経歴など、他の人に見せる情報',
  'character.memo': 'メモ（自分だけが見られる）',
  'character.memoPlaceholder': '自分用のメモ。共有されません',
  'character.image': '画像',
  'character.imageEdit': '編集',
  'character.imageAdd': '画像を追加',
  'character.imageChange': '画像を変更',
  'character.imageRemove': '画像を削除',
  'character.imageView': '画像を拡大表示',
  'character.imageProcessing': '画像を処理しています…',
  'character.imageError': '画像を読み込めませんでした。別の画像をお試しください。',
  'character.crop.title': '画像の位置を調整',
  'character.crop.hint': 'ドラッグで位置、スライダーでズームを調整できます。',
  'character.crop.zoom': 'ズーム',
  'character.delete': 'このキャラクターを削除',
  'character.deleteConfirm': 'キャラクター「{name}」を削除します。元に戻せません。よろしいですか？',
  'character.export': 'ファイルに書き出し',
  'character.exportMemo': 'メモも書き出しに含める',
  'character.import': 'ファイルから読み込み',
  'character.importError': 'キャラクターを読み込めませんでした。ファイルを確認してください。',
  'character.unnamed': '無名のキャラクター',
  'character.actingAs': '{name} として操作中',

  'dice.section': 'ダイスを振る',
  'dice.rollName': 'ロール名',
  'dice.namePlaceholder': 'ロールに名前を付ける（任意）',
  'dice.formula': 'ダイス式',
  'dice.count': '個数',
  'dice.countDec': '個数を減らす',
  'dice.countInc': '個数を増やす',
  'dice.type': '種類',
  'dice.modifier': '補正',
  'dice.modifierDec': '補正を下げる',
  'dice.modifierInc': '補正を上げる',
  'dice.kind': '用途',
  'kind.damage': 'ダメージ',
  'kind.judgment': '判定',

  'pattern.section': '保存したパターン',
  'pattern.name': 'パターン名',
  'pattern.namePlaceholder': '例: 火球の魔法',
  'pattern.save': 'このパターンを保存',
  'pattern.load': '呼び出す',
  'pattern.roll': '振る',
  'pattern.moveUp': '上へ移動',
  'pattern.moveDown': '下へ移動',
  'pattern.delete': '削除',
  'pattern.deleteConfirm': 'パターン「{name}」を削除します。元に戻せません。よろしいですか？',
  'pattern.replaceConfirm': '同名・同用途のパターン「{name}」が既に存在します。置き換えますか？',
  'pattern.none': '「{name}」にはまだパターンがありません。パターンはキャラクターごとに保存されます。',
  'pattern.unnamed': '無名のパターン',
  'pattern.needName': 'パターン名を入力してください',
  'pattern.needCharacter': 'パターンの保存にはキャラクターを選択してください',

  'roll.button': 'ダイスを振る',
  'roll.hidden': '隠しロール（GM）',
  'roll.hiddenHint': 'ON にすると、あなたのロールは出目を伏せて他プレイヤーに通知されます',

  'result.damage': '{value}ダメージ',
  'result.damageNamed': '{name} {value}ダメージ',
  'result.judgment': '{name} 判定の結果 {value}',
  'result.judgmentUnnamed': '判定の結果 {value}',
  'result.hiddenRoll': '{name} が隠しロールを行いました',
  'result.faces': '出目内訳',

  'feed.section': 'ダイス & チャット',
  'feed.filter': 'フィルター',
  'feed.all': 'すべて',
  'feed.rolls': 'ダイス',
  'feed.chat': 'チャット',
  'feed.files': 'ファイル',
  'feed.empty': 'まだ何もありません',
  'feed.emptyRollHint': 'ダイスを振るとここに記録されます。',
  'feed.emptyShareHint': '他のプレイヤーと共有したいときは「ルーム」からルームを作成・参加できます。',
  'feed.emptyOpenRoom': 'ルームを開く',
  'feed.loadOlder': 'これより前を読み込む',
  'feed.jumpToLatest': '最新へ移動',
  'feed.clear': '表示をクリア',
  'feed.compact': 'コンパクト表示',
  'feed.clearConfirm': 'ダイスとチャットのフィードの表示をすべてクリアします。元に戻せません。よろしいですか？',
  'feed.playerDetail': 'プレイヤー情報',
  'feed.playerLeft': 'このプレイヤーは現在ルームにいません。',
  'feed.noBackground': '背景情報は登録されていません。',

  'typing.one': '{names} が入力中…',
  'typing.many': '{names} が入力中…',

  'marker.created': 'ルーム {code} を作成しました',
  'marker.joined': 'ルーム {code} に参加しました',
  'marker.youLeft': 'ルームを退出しました',
  'marker.youClosed': 'ルームを閉じました',
  'marker.gmClosed': 'GM がルームを閉じました',
  'marker.hostLost': 'GM との接続が切れました',
  'marker.playerJoined': '{name} が参加しました',
  'marker.playerLeft': '{name} が退出しました',
  'marker.reconnecting': '接続が切れました。再接続しています…',
  'marker.reconnected': 'ルームに再接続しました',
  'marker.reconnectFailed': '再接続できませんでした',
  'marker.codeChanged': 'ルームコードが {code} に変更されました',

  'room.section': 'ルーム',
  'room.pastSection': '過去のルーム',
  'room.offline': 'オフライン（あなただけ）',
  'room.name': 'ルーム名',
  'room.namePlaceholder': '例: 金曜の冒険',
  'room.create': 'ルームを作成',
  'room.createGmHint':
    'ルームを作成するとあなたが GM になります。GM はルームのホストになるため、GM がオフラインになるとプレイヤーも切断されます。',
  'room.join': 'ルームに参加',
  'room.joinGmHint': 'GM（ホスト）がオフラインになると、プレイヤーも切断されます。',
  'room.back': '戻る',
  'room.gmSection': 'GM',
  'room.codePlaceholder': 'ルームコード',
  'room.codeCreateHint': 'コードを入力すると指定のコードで作成、空のままだと自動生成します。',
  'room.code': 'ルームコード',
  'room.changeCode': 'コードを変更',
  'room.changeName': '名前を変更',
  'room.newCodePlaceholder': '新しいコード',
  'room.codeChangeHint': 'コードを変更するとプレイヤーも自動的に新しいコードへ移ります。',
  'room.copy': 'リンクをコピー',
  'room.copied': 'コピーしました',
  'room.exportHistory': 'ルームをエクスポート',
  'room.importHistory': 'ルームをインポート',
  'room.importHistoryHeading': 'またはファイルから復元',
  'room.importHistoryHint': '過去にエクスポートしたルームのZIPファイルから復元します。あなたがGMとしてホストします。',
  'room.history': '過去のルーム',
  'room.importError': 'ルームファイルを読み込めませんでした',
  'room.leave': 'ルームを退出',
  'room.close': 'ルームを閉じる',
  'room.leaveConfirmGM': 'ルームを閉じると、プレイヤー全員がオフラインになります。よろしいですか？',
  'room.connecting': '接続中…',
  'room.connected': '接続済み',
  'room.players': 'プレイヤー',
  'room.expandAll': 'すべて開く',
  'room.collapseAll': 'すべて閉じる',
  'room.gmBadge': 'GM',
  'room.youBadge': 'あなた',
  'room.shareHint': 'このリンクを他のプレイヤーに共有してください',
  'room.error': '接続に失敗しました。コードを確認してください。',
  'room.gmOffline': 'GM がオフラインです。再接続しています…',
  'room.hostLost': 'GM が不在のためオフラインになりました。',
  'room.codeTaken': 'そのルームコードは別のルームで使用されています。',

  'history.title': '過去のルーム',
  'history.empty': '保存された過去のルームはまだありません。',
  'history.entryCount': '{count} 件の記録',
  'history.unnamed': '名称未設定のルーム',
  'history.open': '過去のルームを開く',
  'history.delete': 'この過去のルームを削除',
  'history.deleteConfirm': 'この過去のルームを削除しますか？元に戻せません。',
  'history.deleteAll': 'すべての過去のルームを削除',
  'history.deleteAllConfirm': '保存されたすべての過去のルームを削除しますか？元に戻せません。',
  'history.backToList': '過去のルーム一覧に戻る',

  'chat.placeholder': 'メッセージを入力',
  'chat.send': '送信',
  'chat.pending': '未送信',
  'chat.viewOriginal': '原文',
  'chat.viewTranslation': '訳文',
  'chat.attach': 'ファイルを添付',
  'chat.mentionEveryone': '全員に通知',
  'chat.removeAttachment': '添付を取り消す',
  'chat.attachTooLarge': 'ファイルが大きすぎます（最大 {max}）',
  'chat.attachFailed': 'ファイルを読み込めませんでした',
  'chat.download': 'ダウンロード',
  'lightbox.close': '閉じる',
  'lightbox.prev': '前の画像',
  'lightbox.next': '次の画像',

  'common.you': 'あなた',
}
