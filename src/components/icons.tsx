 ...
 *
 * UI chrome glyphs (close, trash, feed filters) come from Lucide — a
 * single-weight outline set chosen for a consistent visual vocabulary.
 * The d20 silhouette for the "rolls" filter is from game-icons.net by
 * Delapouite (CC BY 3.0). See docs/CREDITS.md for attribution.
 *
 * Every icon inherits the parent's color via SVG `currentColor` and
 * carries the `icon-svg` class so existing CSS centring rules apply.
 */
 ...
 *  actions where the action removes items from the visible list. */
...
/** Feed filter glyphs — one per `FeedFilter` value. */
...
export function FilterChatIcon() {
  return (
    <MessageSquare
      className="icon-svg"
      size={18}
...
export function FilterFilesIcon() {
  return (
    <Paperclip
      className="icon-svg"
      size={18}
...
/**
 * A 20-sided die silhouette. Sourced from game-icons.net (Delapouite,
 * CC BY 3.0); the original `fill="#000"` is swapped for `currentColor`
 * so the icon takes on the surrounding text color. See
 * `src/assets/icons/dice-twenty-faces-twenty.svg` for the canonical
 * (attributed) source file.
 */
export function D20Icon({ size = 18 }: { size?: number | string }) {
  return (
    <svg
      className="icon-svg"
      width={size}
