...
export interface ChatMessage {
   ...
   *  speaker's display name / background / portrait are pulled from
   *  the per-(player, character) record in `sessionCharacters` rather
   *  than carried inline. */
  characterId: string
  /** GM mark at the moment the message was sent. Carried per-entry so
   *  the feed shows the speaker's GM status as it was at the time, not
   *  as it is now — a player who is currently GM but was a client when
   *  this message was sent is not retroactively badged as GM, and an
