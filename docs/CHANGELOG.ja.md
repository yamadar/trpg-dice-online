...
- v1.116 — **すべてのモーダルダイアログのキーボードフォーカス管理**
  ...
  トークンのキャラクター編集モーダル、画像ピッカー、マップギャラリー、テーブル
  マップのショートカット一覧に適用。すでにフォーカスを閉じ込めていた
  `ConfirmDialog` も同じトラップロジックを共有するようにした。トラップの中核と
  なるループ処理は純粋関数として切り出して単体テストを追加。リスナーは
  ダイアログごとにキャプチャフェーズで登録するため、別のダイアログの上に重なった
  ダイアログ（テーブルマップ上のマップギャラリー、Sheet 上の確認ダイアログなど）
  でも互いに干渉せず自分のフォーカスを閉じ込める。Sheet と切り抜きダイアログは、
