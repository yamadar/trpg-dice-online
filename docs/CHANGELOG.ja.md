...
- v1.118 — **テーブルマップのトークンのキャラクターモーダルから開いた画像ビューア／
  切り抜きダイアログの Escape が、内側のダイアログだけを閉じるよう修正**（モーダル
  全体は閉じない）。テーブルマップで配置済みの PC／NPC トークンのキャラクター
  モーダル（`CharacterInfoModal`、または読み取り専用版）を開き、全画面の画像ビューア
  でポートレートを拡大、または画像切り抜きダイアログを開いてから Escape を押すと、
  内側のダイアログだけでなくトークンモーダル全体まで閉じていた。v1.117 でこれらの
  内側のダイアログにキャプチャフェーズ＋`stopImmediatePropagation` の Escape ハンドラ
  を与えたが、トークンのキャラクターモーダルにも元から同じハンドラがあり、どちらも
  `window` のキャプチャフェーズに登録されているため登録順だけで勝者が決まっていた。
  外側のモーダルが先にマウントされて先に発火し、スタック全体を閉じていた。新しい
  共有フック `useDialogEscape` が、こうしたダイアログすべてに 1 つの正しい実装を
  与える。入れ子のダイアログ（`useDialogFocus` が `data-dialog-focus-trap` を付与）が
  フォーカスを保持し、かつ自分の内側に含まれているとき、祖先のダイアログは処理を
  譲る——`useDialogFocus` が Tab で既に行っている「最も内側が勝つ」受け渡しと同じ
  仕組み——ため、1 回の Escape では最も内側のダイアログだけが閉じる（2 回目の
  Escape でモーダルが閉じる）。`ConfirmDialog`、トークンのキャラクター閲覧／編集
  モーダル、マップギャラリー、全画面の画像ビューア、切り抜きダイアログに適用。
  マップギャラリーは従来どおり自分の入れ子の Lightbox プレビュー（DOM 上は兄弟）を
  先に閉じる。譲るかどうかの判定は純粋関数として単体テスト付き。v1.117 の追補。

- v1.117 — **Sheet 内で開いたダイアログの Escape が Sheet まで閉じてしまう問題を修正**。
  キャラクターの Sheet 内から全画面の画像ビューアでポートレートを拡大したり、
  画像切り抜きダイアログを開いたりして Escape を押すと、内側のダイアログ
  *だけでなく* Sheet 全体まで同時に閉じていた（Sheet 内に表示されるプレイヤー
  詳細カードの画像ビューアも同様）。これらの内側のダイアログは Sheet の DOM
  *子孫* として描画され、それぞれが `stopPropagation` を伴わないバブリングフェーズ
  の `window` Escape→閉じるハンドラを登録していたため、1 回の Escape が `window`
  まで伝播して両方のハンドラが発火していた。画像ビューアと切り抜きダイアログは、
  `ConfirmDialog`・トークンのキャラクターモーダル・マップギャラリーがすでに採用
  しているのと同じパターンで、Escape を*キャプチャ*フェーズで処理し
  `stopImmediatePropagation()` を呼ぶようにした。これにより Escape は最も手前の
  ダイアログだけを閉じ、背後の Sheet は開いたまま残る（2 回目の Escape で Sheet が
  閉じる）。背後に何もない単独利用（フィード／チャット画像から開いた画像ビューア）
  でも、この追加の stop は無害。v1.116 のダイアログフォーカス管理の追補。

- v1.116 — **すべてのモーダルダイアログのキーボードフォーカス管理**
  （`design:accessibility-review`）。v1.115 では 2 つのオンボーディング
  ツアーを開いたときにフォーカスをダイアログへ移したが、他のオーバーレイの
  多くはキーボードフォーカスが背後に取り残されたままで、Tab が裏のページを
  巡回し、ダイアログを閉じるとフォーカスがページ先頭に落ちていた。新しい
  `useDialogFocus` フックが、ダイアログを開いた要素を記録し、フォーカスを内部へ
  移し、Tab／Shift+Tab をダイアログ内のフォーカス可能な要素に閉じ込め（両端で
  ループ）、閉じたときにフォーカスを開いた要素へ戻す（WCAG 2.4.3 フォーカス順序、
  2.1.2 キーボードトラップなし）。アプリ／テーブルマップのチュートリアル、Sheet、
  初回起動の名前入力ゲート、画像切り抜きダイアログ、全画面の画像ビューア、
  トークンのキャラクター閲覧／編集モーダル、画像ピッカー、マップギャラリー、
  テーブルマップのショートカット一覧に適用。すでにフォーカスを閉じ込めていた
  `ConfirmDialog` も同じトラップロジックを共有するようにした。トラップの中核と
  なるループ処理は純粋関数として切り出して単体テストを追加。リスナーは
  ダイアログごとにキャプチャフェーズで登録するため、兄弟として重なるダイアログ
  （テーブルマップ上のマップギャラリー、Sheet 上の確認ダイアログ）は互いに干渉
  せず自分のフォーカスを閉じ込め、別のダイアログの DOM 子孫として描画される
  ダイアログ（Sheet 内から開く切り抜きダイアログ／画像ビューア）では最も内側の
  トラップが優先される。Sheet・切り抜きダイアログ・画像ピッカー・マップギャラリーは、
  2 つのダイアログが同時に表示されても安全なよう、`aria-labelledby` の参照先を
  静的 id から `useId()` に変更した。

- v1.115 — **読みやすさ・操作性のアクセシビリティ改善**
  （`design:accessibility-review`）。**コントラスト:** アクセント面の文字／アイコンが
  全テーマで白だったため、明るいアクセントのテーマで WCAG AA を満たしていなかった
  （forest では「ルームを開く」「次へ」ボタンが約 3.3:1、アクティブなツールバー
  アイコンが約 1.7:1）。`--text-on-accent` をテーマ対応にし（暗いテーマの明るい
  アクセントには黒に近い色、明るいテーマの暗いアクセントには白）、塗りつぶしの
  アクセント要素がすべて AA（文字 4.5:1 以上、アイコン 3:1 以上。全 6 テーマで
  4.57〜7.9:1 を確認）を満たすようにした。daylight と parchment の補助テキスト色
  （`--text-dim`）も、明るい背景で 4.5:1 を満たすよう暗くした（従来は約 3.7〜4.1:1）。
  **操作性:** チャット入力欄、Sheet／NameGate／画像切り抜きダイアログ、プレース
  ホルダーのみだったマップ URL・ライブラリ・キャンバス文字・ギャラリー検索の入力欄に
  アクセシブルネームを追加。2 つのオンボーディングツアーは開いたときにダイアログへ
  フォーカスを移し、Escape で閉じられるようにした。ミニマップの折りたたみボタンを
  18×18 から 24×24 に拡大（WCAG 2.5.8 の最小ターゲットサイズ）。

- v1.114 — **オンボーディングのチュートリアルを刷新**（`design:ux-copy`）。
  テーブルマップのチュートリアルが古くなっていた: 道具パレットの並び順が違い、
  ピンとフォグの「表示／隠す」ブラシが抜け、トークンの向き／HP／状態異常や
  キーボード移動に触れておらず、マップ／グリッド／フォグ／トークン／シーン／
  ライブラリへのツールバー分割より前の内容だった（シーンもミニマップも説明されて
  いなかった）。7 ステップすべてを現在の UI・実際のラベルに合わせて書き直した。
  アプリ全体のツアーにも「共有マップ」ステップを追加（これまでオンボーディング中に
  テーブルマップが登場しなかった）。英語と日本語は完全対応、新規キーは他 17 言語では
  英語にフォールバック（全言語対応はコスト面から後回し）。

- v1.113 — **デザイントークンでボタン・コントロールのスタイルを統一**
  （`design:design-system`）。監査の結果、塗りつぶしの「静かな」ボタンが、UI 構築時に
  定義が外れて以降ずっと未定義だった `--accent-bg` トークンを参照しており、*すべての*
  テーマでハードコードされた紫の色味にフォールバックしていた（forest や ember でもツールバー／ドック／
  プライマリボタンが本来のアクセント色ではなく紫がかっていた）。`--accent-bg` を
  `color-mix(in srgb, var(--accent) 16%, transparent)` で一度だけ定義し、各テーマの
  アクセント色に自動追従するようにした。共通の角丸スケール
  （`--radius-sm/md/lg/pill`）と `--text-on-accent` トークンを追加し、ばらついていた
  コントロールの角丸（4/5/12px）をスケールに揃え、ホバーの明度とフォーカスリングの
  色（一部のトグルが `--accent-2` を使用）を統一、誤解を招くハードコードの色
  フォールバックを削除した。既定（midnight）テーマでの変化はごくわずか（色味のアルファ
  .12/.15/.18 を 16% に統一）で、他 5 テーマで塗りつぶしのコントロールが正しく色付く
  ようになった。

- v1.112 — **「マップとグリッド」を 2 つのカテゴリに分離**。デザインクリティーク
  （`design:design-critique`）の結果、結合パネルでは背景マップの選択が 7 つの
  グリッド設定の下に埋もれており、そもそもマップを設定できることが分かりづらかった。
  マップとグリッドをそれぞれ独立したトップレベルのカテゴリに分離: **背景マップ**
  （専用の風景アイコンで一覧の先頭）は 4 つのソース（アップロード／ギャラリー／
  URL／プリセット）に直結し、**グリッド**はグリッド設定を持つ。アイコンの並びは
  セットアップの流れに沿って マップ → グリッド → フォグ → トークン → シーン →
  ライブラリ。

- v1.111 — **テーブルライブラリパネルの整理**。デザインクリティーク（`design:design-critique`
  スキル）の結果、パネル冒頭に低コントラストの説明文が約8行並び、各エントリに
  塗りつぶしの「テーブルを置換」ボタンが繰り返し表示されて、肝心の保存済み一覧が
  埋もれていた。説明文（ライブラリとは何か／保存・読み込みの動作）を「？」ヘルプ
  トグル（既定は折り畳み）に集約し、セクションごとの「意味」行はそこへ統合。各
  エントリの 2 つのロード操作は同じ控えめなアウトラインボタンに揃え、一覧を
  見やすくした。機能の変更はなし。

- v1.110 — **モバイルでトークン情報パネルを中央表示**。スマホ（720px 以下）では、
  トークン情報パネルをトークンに固定せず、キャンバスの中央に表示するようにした
  （高さ上限つき・スクロール可能）。小画面では指の下や画面外になりがちだったため。
  PC 表示はトークンに固定する従来どおり。

- v1.109 — **テーブルマップの細かな修正（4件）**。(1) ミニマップがトークン情報
  パネルを覆わないように、z-index をその下へ。(2) 追加直後の NPC で、名前を
  入力する前に画像（やメモ）を変更しても反映されるように修正。更新ガードが
  名前が空の間はすべての編集を弾いていたため画像が捨てられていた。判定は
  純粋・単体テスト済みの `tabletop/npc.ts`（`nextNpcDef`）に切り出し、
  「名前を明示的に空にする操作」だけを弾くようにした。(3) NPC ライブラリの
  「追加」ボタンを見出し行へ移し、各エントリの「配置」ボタンと右端を揃えた。
  (4) トークン上の状態異常の絵文字を下へ少しずらし、黒背景チップの中央に
  収まるようにした（絵文字は行ボックス内で上寄りに描画されるため）。

- v1.108 — **トークンポップオーバーのタップ可能なキャラクターカード**。PC トークンの
  名前欄を、キャラクター情報カードを縮小したカード（丸い立ち絵＋名前）に変更し、
  カード自体をタップでキャラクター情報を開けるようにした。アクセント色のヒントと
  情報アイコンで、タップできることが直感的に分かる。独立した「キャラクター情報」
  ボタンは廃止し、ポップオーバーも少し短くなった。GM トークンは編集可能なラベルの
  まま。キャラクターを持たない「プレイヤーとして」配置のトークンは静的なカードを表示。

- v1.107 — **ミニマップがフォグオブウォーを反映**。これまでミニマップは霧の下も
  含めてマップ全体とすべてのトークンを表示しており、GM が隠した領域がプレイヤーに
  ネタバレしていた。未開示セルに霧を描画し（プレイヤーには不透明、GM には半透明で
  本体キャンバスと同じ）、非 GM では霧の中にあるトークン / ピンのドットを非表示に
  するようにした。オーバーレイは小さなサンプリング用キャンバスでスクエア / ヘックス
  双方を 1 経路で扱う。開示判定は純粋・単体テスト済みの `fogHidesWorldPoint`
  （`tabletop/minimap.ts`）。

- v1.106 — **トークンポップオーバーの圧縮**。向き・HP・状態異常が増えたことで
  トークン編集ポップオーバーが非常に縦長になっていた（常時開いた 3×3 の向き
  コンパスと 12 個の状態異常チップ）。サイズ・向き・HP・状態異常を折り畳み式の
  1 行表示にし、各行に現在値（サイズの数値・向きの矢印・`現在/最大`・状態異常の
  絵文字）を出し、タップでその編集 UI を展開（同時に開くのは 1 つ）するように
  した。ツールバーの「1 パネルずつ開く」メタファに合わせている。

- v1.105 — **ミニマップのクラッシュ修正＋ピンの気づきやすさ**。マップの無い
  シーンでミニマップをドラッグするとカメラが遠くへ飛んでいき（タブが固まる）
  ことがあった。原因はミニマップの世界フレームに現在のビューポートが含まれて
  いたこと——リセンターするたびにフレームが動き、同じポインタ位置がさらに遠い
  点へ写像される暴走ループだった。フレームをパン不変（マップ範囲 → トークンの
  バウンディングボックス → 原点中心・ビューポートサイズのフォールバック）に
  変更してドラッグを安定化し、`recenterOn` も非有限値を無視するようにした。
  さらにミニマップ上にピンを表示し、ビューポート外でピンが置かれた場合は画面端に
  その方向を指す色付きの矢印（タップで移動）を出すようにして、見逃さないように
  した。純粋関数 `offscreenEdgePosition`（`tabletop/ping.ts`）と書き直した
  `minimapWorldBounds`（`tabletop/minimap.ts`）はいずれも単体テスト済み。

- v1.104 — **シーンとライブラリの整理**。シーン導入後、テーブルライブラリと
  シーン切替の役割が紛らわしく重なっていたため、保存とロードを明示化した。
  保存時に**範囲**（*このシーン* か *テーブル全体（全シーン）*）を選べるように
  し、全シーンを暗黙に巻き込まないようにした。**テンプレート**は PC トークンと
  ペン描画を*全保存シーン*から除去するよう修正（従来は現在シーンのみで、
  非アクティブなシーンに PC が残っていた）。各エントリは 2 通りでロードできる
  —**テーブルを置換**（全入れ替え）か**シーンとして追加**（現在のシーンを
  保持したままエントリのシーンを差し込む）で、ライブラリの素材を進行中の
  ゲームへ持ち込める。パネルにはライブラリ＝端末横断で再利用できる棚、
  シーン＝このゲーム内だけ、と明記。複数シーンのエントリにはシーン数バッジを
  表示し、日本語 UI は英語の "Snapshot" に合わせて「スナップショット」に統一。
  範囲解決のロジックは純粋・単体テスト済みの `tabletop/scenes.ts`
  （`allScenes` / `currentSceneOnly` / `stripTemplateScenes` /
  `appendScenes`）。

- v1.103 — **ミニマップ**。現在シーンの折り畳み可能な隅の俯瞰 — 背景マップ
  （またはフィットした空領域）・トークンごとのドット・現在ビューポートを示す
  矩形。ミニマップをクリック / ドラッグすると、その位置へカメラを
  センタリングする。幾何（ワールド矩形のボックスへのフィット、ワールド↔
  ミニマップの写像、ワールド境界の決定）は純粋・単体テスト済みの
  `tabletop/minimap.ts`。これで当初の Phase 2 テーブルマップ一覧はルーラーを
  除いて完了。

- v1.102 — **1 セッション複数マップ（シーン）**。GM は複数のシーン（各自の
  マップ・グリッド・トークン・注釈・フォグ）を持ち、新しい「シーン」ツール
  バーカテゴリからアクティブを切り替えられる（追加 / 改名 / 削除。最後の
  1 つは削除不可）。NPC ライブラリと PC スポーン地点はシーン共通のまま。
  シーンはワイヤ上 GM 専用で、非アクティブなシーンはブロードキャスト前に
  除去される。切替は新しい `tabletopState` の配信とマップ再ストリームだけで、
  クライアントは現在のシーンのみを写す。シーンはリロード後も保持され、
  ルームのエクスポート / インポートを往復する（各シーンのマップは
  `attachments/maps/` に外部化）。純粋・単一情報源の `tabletop/scenes.ts`
  （現在シーンは top-level、`scenes[]` が残りを保持）で実装し、シーン番号は
  単調増加。

- v1.101 — **キーボード移動・ショートカットの完全対応**。テーブルマップに
  フォーカスがあり入力欄を編集していないとき、矢印キーで選択トークンを
  1 マスずつ移動、`1`〜`5`（V/T/P/E/G）でツール切替、`+` / `-` / `0` で
  ズーム拡大 / 縮小 / リセット、`f` で選択へセンタリング、`[` / `]` で操作
  可能トークンを巡回、`Delete` で削除、`Esc` で選択解除→閉じる、`?` で
  ショートカット早見表をトグルできるようにした。移動はドラッグと同じ
  `canMoveToken` 権限に従う。キー→意図の対応は純粋・単体テスト済みの
  `tabletop/keymap.ts`。

- v1.100 — **HP バー & 状態異常**。トークンに任意の HP プール（現在 / 最大）
  を持たせ、トークン下部に色分けバー（緑→黄→赤）で描画できるようにした。
  また固定カタログ（毒・スタン・睡眠・恐怖・魅了・燃焼・凍結・祝福・防御・
  加速・出血・ダウン）から選ぶ状態異常をトークン上部に絵文字バッジで表示する。
  どちらもトークンダイアログ（HP 入力 ＋ チップ）で編集し、移動 / 向きと同じ
  `canMoveToken` 権限で、ホスト検証付きの `tokenHpRequest` /
  `tokenStatusRequest`（ホスト側でクランプ / サニタイズ）を通す。HP / 状態の
  計算は `tabletop/vitals.ts`（単体テスト済み）にあり、いずれもリロード
  サニタイザを往復する。

- v1.99 — **トークンの向き**。トークンに任意の向きを持たせられるように
  した。トークンダイアログに 8 方位コンパス（＋クリア）を追加し、キャンバス
  ではトークンのすぐ外側に小さな方向矢印を描く。向きは移動 / サイズ変更と
  同じ権限（自分の PC または GM）で、新しい `tokenFacingRequest` を
  `canMoveToken` で検証してホスト側で適用する。角度計算（正規化・画面空間
  ベクトル・矢じりの幾何）は `tabletop/facing.ts`（単体テスト済み）にある。
  あわせて長年のリロードギャップも解消し、`sanitizeStoredTabletop` が
  トークンの `size` / `note` / `privateNote` / `facing` を往復するように
  なった（従来は live 同期では残るが IndexedDB 再読込で失われていた）。

- v1.98 — **ピング（「ここ見て」）マーカー**。参加者は誰でも左パレットの
  新しいピンツールを選び、マップをタップして一時的な注目マーカー — 本人の
  プレイヤー色で広がるさざ波と名前ラベル — を置ける。全員に配信され、約 2.6 秒で
  消える。ピングは意図的に**揮発性**で、テーブル state には一切入らないため、
  永続化されず、welcome snapshot にも含まれず、エクスポートにも乗らない。
  クライアントはホスト検証付きの `pingRequest`（送信者 id を押し直し・座標を
  検証）を送り、ホストが `ping` として再配信する。純粋なアニメーション / 検証
  計算は `tabletop/ping.ts`（単体テスト済み）にあり、マーカーは新しい
  `PingMarker` 描画レイヤで自己アニメーションする。

- v1.97 — **トークン配置を4列グリッド折り返し方式に変更**。従来は配置の
  たびに1マス右へずれる一列配置だったため、参加者が増えると可視範囲外へ
  トークンが散らばっていた。4列で折り返して次の行へ進む方式に変更し、
  何個配置してもクラスターがコンパクトに収まるようにした。

- v1.96 — **トークンパネル・権限モデルの大規模刷新**（PR #187〜#188）。
  トークンサイドバーを「マップ上のトークン」（常時表示・クリックでハイライト）
  と折り畳み可能な「追加・準備」の2セクションに再編。自分のトークンは
  アクセントカラーで強調表示、操作できないトークンはキャンバス上で
  opacity 0.8 で薄く表示。全トークンに全員が編集可能な**公開メモ**を追加し、
  GMトークンにはワイヤに乗らない**GMメモ（非公開）**も追加。権限マトリクスを
  整理：PL は自分のPCトークンを移動・サイズ変更・削除可能（ホスト検証付き
  リクエスト）。任意のトークンをタップして開けるダイアログを実装（GM は
  編集、非操作者は読み取り専用＋「操作できる人」表示）。サイズ変更の
  ドリフトバグ修正（`snapResizeToGrid`）。「Edit token」→「NPC token」に改称。
  ダイスロールでテーブルマップのチャットアイコンに未読ドット表示。
  ロール時にロールしたプレイヤーのトークンからサイコロアイコンがアニメーション。
  チュートリアルボタンを opacity 0.5 で控えめに。

- v1.95 — **画像ピッカーの確定操作を統一**。シングルクリック選択＋
  フッターボタン確定（マップギャラリーと同じ「キャンセル / この画像を使う」
  方式）に変更。ポータル化したピッカー内のクリックがモーダルを閉じて
  しまうバグを修正（ポップオーバーの外側クリック判定から
  `.map-gallery-layer` を除外）。

- v1.94 — **トークンポップオーバーの細部改善**。「+」で追加した新規NPCは
  タイトルが「NPC を作成」、既存NPCの編集は「NPC を編集」と分岐。
  PCトークンのポップオーバーにキャラクター名を表示。NPC トークンの
  画像ピッカーを `document.body` にポータルして全画面幅に。

- v1.93 — **テーブルマップUI改善** 11 項目：吹き出しの尖り（tail）位置修正、
  テーブルマップドックのチャットアイコンに未読ドット、テーブルマップ内の
  チャットスクロール修正、背景マップ「差し替え」ボタンの重複解消と
  「クリア」のアイコン化、NPC「画像を変更」をサイズの下に移動、
  NPC「画像を変更」ダイアログの幅拡大、NPCライブラリの名前入力を廃止して
  追加をアイコンのみに、キャラクター新規作成時に名前にフォーカス、
  NPCライブラリ・配置済みトークンに▲▼並び替えコントロール追加、
  大きいトークン（2マス以上）のエフェクトリングを内側でなく周囲に表示、
  GMの非操作キャラのトークンが他の参加者に見えない同期バグを修正。

- v1.92 — **ドキュメント更新**：ヘックスグリッドとフォグ・オブ・ウォーの
  仕様を `REQUIREMENTS.md` / `REQUIREMENTS.ja.md` に追記。機能変更なし。

- v1.91 — **トークン・NPCエディタのUI刷新**：トークン削除ラベルの変更、
  PCトークンのポップオーバーからキャラクター情報モーダルを直接開けるように、
  NPCライブラリのエントリがポップアップエディタで編集可能に、
  NPCトークンの「画像を変更」を幅広モーダルに。

- v1.90 — **操作キャラを切り替えるとPCトークンの画像が消えるバグを修正**。
  GMの操作キャラ切り替え時、直前まで操作していたキャラのトークン画像が
  消失していた。配置済みPCトークンの画像をキャラクターレコードの変化時だけ
  更新するよう修正。

- v1.89 — **ヘックスグリッドで霧ブラシが動作しない不具合を修正**
  （v1.88 と同じ根本原因・ブラシジェスチャのガード部分）。4 か所の
  `grid.kind === 'square'` ガードを緩和し、スクエアとヘックスの両方で
  霧ブラシが機能するようにした。

- v1.88 — **ヘックスグリッドでフォグ・オブ・ウォーが塗れない不具合を
  修正**。霧ブラシのジェスチャ開始判定とセル塗り判定が
  `grid.kind === 'square'` に限定されていたため、ヘックスでは
  「霧を晴らす / 霧をかける」を選んでも塗れなかった（マウス・タッチ
  共通。モバイルで顕在化）。セル特定 (`cellFromWorld`)・描画
  (`FogLayer`)・状態更新 (`paintFog`) はすでにヘックス対応済みだった
  ので、4 か所のガードを「グリッドなし (`'none'`) のときだけ無効」に
  緩めて対応した。
