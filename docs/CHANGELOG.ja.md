...
- v1.113 — **デザイントークンでボタン・コントロールのスタイルを統一**
  （`design:design-system`）。監査の結果、塗りつぶしの「静かな」ボタンが定義されて
  いない `--accent-bg` トークンを参照しており、*すべての*テーマでハードコードされた
  紫の色味にフォールバックしていた（forest や ember でもツールバー／ドック／
  ...
  フォールバックを削除した。既定（midnight）テーマでは見た目の変化なし、他 5 テーマで
  正しく色付くようになった。
