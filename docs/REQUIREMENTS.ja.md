...
- トークンは 2 種類。
  - **PC トークン**: セッションキャラクター（`sessionCharacters`）から
    生成され、画像はキャラクターポートレートを再利用する。当該キャラの
    オーナーと GM が動かせる。追加はキャラのセッション参加時に自動、
    削除は GM のみ。
  - **GM 専用トークン**: PC に紐付かない（NPC・モンスター・障害物用）。
    GM のみ追加・移動・削除できる。画像はキャラクターポートレートと
    同じ縮小フロー（長辺 2560px・約 2MB）でアップロードする。
...
- [ ] GM が背景マップをアップロードし、参加者全員に同期される
- [ ] スクエアグリッドのセルサイズ・オフセット・色・透明度を調整できる
- [ ] セッションのキャラクターから PC トークンが生成され、オーナーと GM が動かせる
- [ ] GM 専用トークンを GM が追加・移動・削除できる
- [ ] パン・ズームがタッチとマウスの両方で動作する
- [ ] モバイルでテーブルマップが全画面表示され、フィードが下からスワイプアップで開く
- [ ] リロードしてもテーブルマップの状態が復元される
