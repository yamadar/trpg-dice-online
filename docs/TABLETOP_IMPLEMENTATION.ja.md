...
- **ミニマップ**: 現在シーンの折り畳み可能な隅の俯瞰図 — マップ（または
  ...
  **パン不変**（マップ範囲 → トークンの bbox → 原点中心・ビューポート*サイズ*の
  ...
  リセンターがフレームに帰還してマップ無しシーンでカメラが暴走する。フォグオブ
  ウォーも反映する: サンプリング用オーバーレイ canvas で未開示セルを着色（非 GM は
  不透明、GM は 0.5）し、非 GM では霧の中のトークン / ピンドットを省く。開示判定は
  純粋関数 `fogHidesWorldPoint`。
...
| `TableToolbar.tsx` | 右端のアイコンカテゴリパネル: **背景マップ**（4 タブのマップソース）と **グリッド**（グリッド設定）を独立項目に・**フォグ**・**トークン**（マップ上＋追加・準備）・**シーン**・**ライブラリ** |
...
トークンをタップすると、その画面位置にアンカーした DOM popover が開く（モバイル
＝720px 以下では、トークンに固定せずキャンバス中央に表示する。高さ上限つき・
スクロール可能。小画面では指の下や画面外になりがちなため）。**PC
トークン**では識別行が*キャラクター情報カードのミニ版*（丸い立ち絵＋名前）で、
タップするとキャラクター情報を開く（アクセント色のヒント＋情報アイコンで操作を
示唆。旧来の名前行＋独立した「Character info」ボタンを置き換えた）。**GM
トークン**は編集可能なラベル（オペレーター）/ ラベル表示（閲覧者）のまま。GM
（または PC トークンのオーナー）はさらに: 公開メモ・NPC 画像変更・削除・
**非公開 GM メモ**（host のみ）、そして 4 つの属性 — **サイズ / 向き / HP /
状態異常** — を折り畳み式の 1 行で表示する（各行に現在値を出し、タップで編集 UI を
展開。同時に開くのは 1 つだけなので popover が縦長にならない）。非オーナーは
読み取り専用ビュー（サイズ値のみ。他はトークン自体に表示される）で、操作を許された者
（GM とオーナー）も併記される。

### 4.3 ツールバー & ツール

`TableToolbar` は右端のアイコンカテゴリ（同時に開くのは 1 つ）。セットアップの
流れ順に並ぶ: **背景マップ**（GM）— 4 タブのマップソース（Upload / Gallery /
URL / Preset）＋現在マップのクリア。グリッド設定の下に埋もれず発見しやすいよう
トップレベルの独立項目にした; **グリッド**（GM）— 種別 / サイズ / 原点 / 色 /
不透明度 / スナップ; **フォグ**（GM）— 有効化＋全面化 / 全公開＋左ブラシへの案内;
