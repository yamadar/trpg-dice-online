...
公開メモ・NPC 画像変更・削除・**非公開 GM メモ**（host のみ）・「Character info」
起動、そして 4 つの属性 — **サイズ / 向き / HP / 状態異常** — を折り畳み式の
1 行で表示する（各行に現在値を出し、タップで編集 UI を展開。同時に開くのは 1 つ
だけなので popover が縦長にならない）。非オーナーは読み取り専用ビュー
（サイズ値のみ。他はトークン自体に表示される）で、操作を許された者
（GM とオーナー）も併記される。
